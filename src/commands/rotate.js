...
The amount (in degrees) to rotate the image clockwise. Negative values work as well.`,
    async execute(message, { args }) {
        if (message.guild && !(await checkPermissions(message, 'ATTACH_FILES')))
            return;
        const msg = await resolveMessage(message, args.length > 1 ? args[0] : undefined, args.length === 3 ? args[1] : undefined);
        if (!msg)
            return;
        const attachment = msg.attachments.first();
        if (!attachment) {
            await message.reply('that message doesn’t have any attachments! Noot noot.');
            return;
        }
        ...
        try {
            ...
            handleError(message.client, error, `Error rotating image ${attachment.url}`, message, 'there was an error rotating the image!');
            return;
        }
        message.channel.stopTyping();
        await typingPromise;
        await message.channel.send({
            content: msg.attachments.size > 1
