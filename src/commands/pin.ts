...
The ID of the message to pin. Defaults to the last message (excluding the one to execute this command) you sent in this server.`,
  async execute(message, {args}) {
    if (message.guild ? message.guild.ownerID !== message.author.id : message.author.id !== me) {
      message.reply('This command can only be used by the guild or bot owner!')
      return
    }

    if (message.guild && !checkPermissions(message, 'MANAGE_MESSAGES')) return
    ...
    if (id === undefined) {
      message.reply('you have not provided an ID and I went offline since you sent your last message (or you haven\u2019t sent another message)!')
      return
    }

    if (!/^\d{17,19}$/u.test(id)) {
      message.reply(`${id} isn\u2019t a valid ID!`)
      return
    }

    const msg = message.guild ? allGuildMessages!.get(id) : message.channel.messages.cache.get(id)
    if (!msg) {
      message.reply(`the message with ID ${id} couldn\u2019t be found!`)
      return
    }
