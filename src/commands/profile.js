...
import { checkPermissions, createDateFormatter, formatBoolean, imageField, resolveUser } from '../utils.js';
                ...
                name: startCase(activity.type) +
                    (activity.type === 'LISTENING'
                        ...
                        : activity.type === 'COMPETING'
                            ? ' in'
                            : ''),
                value: activity.type === 'CUSTOM_STATUS'
                    ? (activity.emoji
...
const memberInfoFields = ({ displayColor, displayHexColor, joinedAt, nickname, premiumSince, roles, voice: { channel, deaf, mute, serverDeaf = false, serverMute = false, streaming } }, formatDate) => [
    ...(joinedAt
...
You can mention the user or use their tag (for example \`Username#1234\`).`,
    async execute(message, { input }, database) {
        ...
        if (!user)
            return;
        const formatDate = createDateFormatter(await fetchTimeZone(database, message.author));
        const { bot, tag } = user;
