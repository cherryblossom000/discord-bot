import { GuildDefaultMessageNotifications, GuildPremiumTier, GuildVerificationLevel, SlashCommandBuilder, channelMention } from 'discord.js';
import { fetchTimeZone } from '../../database.js';
import { checkPermissions, createDateFormatter, fetchGuild, formatBoolean, inlineField as f, imageField, pascalToStartCase, screamingSnakeToStartCase } from '../../utils.js';
const allSystemChannelSuppressFlags = [
    'SuppressJoinNotifications',
    'SuppressPremiumSubscriptions',
    'SuppressGuildReminderNotifications',
    'SuppressJoinNotificationReplies'
];
const channelFieldData = (fieldName, channelId, getSuffix = () => '') => channelId === null
    ? []
            ...
            embeds: [
                {
                    ...
                    fields: [
                        ...
                        ...(vanityURLCode === null
                            ? []
                            : [
                                f('Vanity URL', `https://discord.gg/${vanityURLCode} (${vanityURLUses ?? 0}) uses`)
                            ]),
                        ...(afkChannelId === null
                            ? []
                            : [
                                await client.channels
                                    .fetch(afkChannelId)
                                    .then(afkChannel => f('AFK Channel', afkChannel.name + afkTimeoutText()))
                                    .catch(() => f('AFK Channel Id', afkChannelId + afkTimeoutText()))
                            ]),
                        ...(systemChannelFlags.has(allSystemChannelSuppressFlags)
                            ? []
                            : channelFieldData('System Messages Channel', systemChannelId, () => ` (${systemChannelFlags
                                .missing(allSystemChannelSuppressFlags)
                                .map(string => pascalToStartCase(string.slice(8)))
                                .join(', ')})`)),
                        ...(widgetEnabled ?? false
                            ? channelFieldData('Widget Channel', widgetChannelId)
                            : []),
                        ...channelFieldData('Community Updates Channel', publicUpdatesChannelId),
                        ...channelFieldData('Rules Channel', rulesChannelId),
                        ...(premiumSubscriptionCount ?? 0
                            ? [
                                f('Sever Boost Status', `${premiumTier === GuildPremiumTier.None
                                    ? 'No Server Boost'
                                    : `Level ${premiumTier}`} (${premiumSubscriptionCount} boosts)`)
                            ]
                            : []),
                        f('Default Notifications', defaultMessageNotifications ===
                            GuildDefaultMessageNotifications.AllMessages
                            ? 'All Messages'
                            : 'Only @mentions'),
                        f('Verification Level', verificationLevel === GuildVerificationLevel.VeryHigh
                            ? 'Highest'
