...
database.on('error', async error => {
  console.error(error)
  await handleError(client, error, 'Keyv connection error:')
})
...
// Import commands, start server and login to Discord
;(async (): Promise<void> => {
  // Initialise commands
  await Promise.all([
...
})().catch(e => console.error('Error importing commands, starting server, or logging in:', e))
